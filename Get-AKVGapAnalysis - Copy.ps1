        ...
        foreach ($vault in $rbacVaults | Select-Object -First 2) {
            ...
            $html += @"
                <div style="background: #f8f9fa; padding: 10px; border-radius: 5px; margin: 10px 0;">
                    ...
                    <strong>Compliance Scores:</strong> CIS: $($vault.CISComplianceScore)%, NIST: $($vault.NISTComplianceScore)%, ISO: $($vault.ISOComplianceScore)%, MS: $($vault.MSComplianceScore)%<br>
...
"@
    ...
    <div class="vault-details">
        ...
        <table>
            <tr><th>Vault Name</th><th>Subscription</th><th>Location</th><th><a href="https://www.cisecurity.org/benchmark/azure/" target="_blank" title="CIS Azure Foundations Benchmark">CIS Score</a></th><th><a href="https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/r5/" target="_blank" title="NIST Cybersecurity Framework">NIST Score</a></th><th><a href="https://www.iso.org/standard/54534.html" target="_blank" title="ISO 27001 Information Security">ISO Score</a></th><th><a href="https://learn.microsoft.com/en-us/azure/key-vault/general/security-baseline" target="_blank" title="Microsoft Security Baseline">MS Baseline</a></th><th>Risk Level</th><th>Diagnostics</th><th>RBAC</th><th>Private Endpoints</th><th>Security Gaps</th></tr>
"@
    ...
    foreach ($vault in $AnalysisResults) {
        ...
        $html += @"
            <tr>
                ...
                <td>$($vault.CISComplianceScore)%</td>
                <td>$($vault.NISTComplianceScore)%</td>
                <td>$($vault.ISOComplianceScore)%</td>
                <td>$($vault.MSComplianceScore)%</td>
    ...
    <script>
        function showDetailsModal(id, type) {
            ...
            if (type === 'platform') {
                ...
                html += '<h3>Vault: ' + (data.VaultName || id) + '</h3>';
                html += '<p><strong>Subscription:</strong> ' + (data.SubscriptionName || data.SubscriptionId) + '</p>';
                html += '<p><strong>Location:</strong> ' + (data.Location || 'N/A') + '</p>';
                html += '<p><strong>Secrets / Certs / Keys:</strong> ' + (data.SecretsCount||0) + ' / ' + (data.CertificatesCount||0) + ' / ' + (data.KeysCount||0) + ' (Total: ' + (data.TotalAssets||0) + ')</p>';
                html += '<p><strong>Diagnostics enabled:</strong> ' + ((data.Diagnostics && data.Diagnostics.HasDiagnostics) ? '✅' : '❌') + '</p>';
                html += '<p><strong>RBAC enabled:</strong> ' + ((data.AccessControl && data.AccessControl.RbacEnabled) ? '✅' : '❌') + '</p>';
                html += '<p><strong>Private endpoints:</strong> ' + ((data.NetworkSecurity && data.NetworkSecurity.HasPrivateEndpoints) ? '✅' : '❌') + '</p>';
                html += '<p><strong>Rotation needs:</strong> ' + ((data.Rotation && data.Rotation.ManualRotationNeeded) ? data.Rotation.ManualRotationNeeded : 0) + '</p>';
                html += '<p><strong>Compliance:</strong> CIS ' + (data.Compliance ? data.Compliance.CIS : 'N/A') + '%, NIST ' + (data.Compliance ? data.Compliance.NIST : 'N/A') + '%</p>';
                ...
                html += '<details style="margin-top:10px;"><summary style="cursor:pointer;">Raw vault JSON</summary><pre style="white-space:pre-wrap;background:#f8f9fa;padding:10px;border-radius:4px;">' + JSON.stringify(data,null,2) + '</pre></details>';
            }
