...
.PARAMETER TestMode
    [switch]$Resume,
    # Non-invasive test helper: simulate an interrupt after N processed vaults.
    # ONLY for testing. If provided and >0, the script will behave as if a CTRL-C
    # occurred after that many vaults have been processed (saves checkpoint and sets stop flag).
...
"@
    ...
    Write-Log "[HTML] Preparing quick wins section (collecting and deduplicating)" -Level "INFO"
...
"@
    ...
    Write-Log "[HTML] Quick wins section prepared: $($uniqueWins.Count) unique wins" -Level "INFO"
...
"@
    ...
    if ($accessPolicyVaults.Count -gt 0) {
        Write-Log "[HTML] Preparing examples for access-policy vaults: $($accessPolicyVaults.Count) candidates" -Level "INFO"
        $html += @"
...
"@
    ...
    Write-Log "[HTML] Example sections prepared (examples: $exampleCount)" -Level "INFO"
