...
function Save-Checkpoint-Atomic {
    ...
    try {
        ...
        $json = $State | ConvertTo-JsonCompact -InputObject -Depth 5
...
function Write-AtomicJson {
    ...
    $tmp = "$Path.tmp"
    try {
        $json = (Normalize-ForJson $Object | ConvertTo-JsonCompact -InputObject -Depth $Depth)
        $json | Out-File -FilePath $tmp -Encoding UTF8 -Force
...
function Build-MasterCsvRecord {
    ...
    $secretsCountVar = NullCoalesce(($Analysis.SecretInventory.SecretsCount -as [int]),0)
    $certificatesCountVar = NullCoalesce(($Analysis.SecretInventory.CertificatesCount -as [int]),0)
    $keysCountVar = NullCoalesce(($Analysis.SecretInventory.KeysCount -as [int]),0)
    $totalAssetsVar = NullCoalesce(($Analysis.SecretInventory.TotalAssets -as [int]),($secretsCountVar + $certificatesCountVar + $keysCountVar))
    }
    ...
    $recentlyRotatedVar = NullCoalesce(($Analysis.RotationAnalysis.RecentlyRotated -as [int]),0)
    $manualRotationNeededVar = NullCoalesce(($Analysis.RotationAnalysis.ManualRotationNeeded -as [int]),0)
    $neverRotatedVar = NullCoalesce(($Analysis.RotationAnalysis.NeverRotated -as [int]),0)
    }
    ...
    $complianceScore = NullCoalesce(($Analysis.ComplianceScore -as [int]),0)
    $cisScore = NullCoalesce(($Analysis.CISComplianceScore -as [int]),0)
    $nistScore = NullCoalesce(($Analysis.NISTComplianceScore -as [int]),0)
    $isoScore = NullCoalesce(($Analysis.ISOComplianceScore -as [int]),0)
    $msScore = NullCoalesce(($Analysis.MSComplianceScore -as [int]),0)
    $riskLevel = NullCoalesce(($Analysis.RiskLevel -as [string]),'Unknown')
    ...
    if ($Analysis.Vault -and $Analysis.Vault.Tags) {
        $tagsJsonCompact = (Normalize-ForJson $Analysis.Vault.Tags | ConvertTo-JsonCompact -InputObject -Depth 2)
    } else {
        $tagsJsonCompact = '{}'
    }
    ...
    if ($Analysis.Extra) {
        ...
        try { $resourceLockTypes = ($Analysis.Extra.ResourceLocks | ForEach-Object { $_ } ) -join ';' } catch { $resourceLockTypes = NullCoalesce(($Analysis.Extra.ResourceLockTypes -as [string]),'') }
    try { $networkIpRules = $Analysis.Extra.NetworkAcls.IpRules -join ';' } catch { $networkIpRules = NullCoalesce(($Analysis.Extra.NetworkIpRules -as [string]),'') }
    try { $networkVNetRuleIds = $Analysis.Extra.NetworkAcls.VirtualNetworkRules -join ';' } catch { $networkVNetRuleIds = NullCoalesce(($Analysis.Extra.NetworkVNetRuleIds -as [string]),'') }
    try { $networkDefaultAction = $Analysis.Extra.NetworkAcls.DefaultAction -as [string] } catch { $networkDefaultAction = NullCoalesce(($Analysis.Extra.NetworkDefaultAction -as [string]),'') }
    try { $networkBypass = $Analysis.Extra.NetworkAcls.Bypass -as [string] } catch { $networkBypass = NullCoalesce(($Analysis.Extra.NetworkBypass -as [string]),'') }
            ...
            if ($norm -is [System.Collections.IEnumerable] -and -not ($norm -is [string])) {
                ...
                foreach ($item in $norm) {
                    ...
                    if ($item.PSObject -and ($item.PSObject.Properties.Name -contains 'RoleDefinitionName' -or $item.PSObject.Properties.Name -contains 'PrincipalId')) {
                        ...
                        try { $parts += (ConvertTo-JsonCompact -InputObject $item -Depth 2) } catch { $parts += $item.ToString() }
                    }
                }
        ...
        } catch { $roleAssignmentsResolved = NullCoalesce(($roleAssignmentsResolved -as [string]),'') }
    }
        ...
        if ($roleAssignmentsResolved -is [System.Collections.IEnumerable] -and -not ($roleAssignmentsResolved -is [string])) {
            ...
            foreach ($it in $roleAssignmentsResolved) {
                ...
                if ($it -is [PSObject] -or $it -is [hashtable]) {
                    try { $parts += (ConvertTo-JsonCompact -InputObject $it -Depth 2) } catch { $parts += $it.ToString() }
    ...
    } catch { $roleAssignmentsResolvedCsv = NullCoalesce(($roleAssignmentsResolved -as [string]),'') }
        ...
        if ($vaultJsonPath -and $vaultJsonPath -ne '') {
            $vaultExport = @{
                ...
                PrivateEndpointIdsNormalized = $privateEndpointIdsNormalized
                        ...
                        AccessPoliciesDetails = NullCoalesce(($Analysis.Extra.AccessPoliciesDetails -as [string]),'')
                        AccessPolicies_Permissions_Keys = NullCoalesce(($Analysis.Extra.AccessPolicies_Permissions_Keys -as [string]),'')
                        AccessPolicies_Permissions_Secrets = NullCoalesce(($Analysis.Extra.AccessPolicies_Permissions_Secrets -as [string]),'')
                        AccessPolicies_Permissions_Certificates = NullCoalesce(($Analysis.Extra.AccessPolicies_Permissions_Certificates -as [string]),'')
                        ProvisioningState = NullCoalesce(($Analysis.Extra.ProvisioningState -as [string]),'')
                        LogAnalyticsWorkspaceIds = NullCoalesce(($Analysis.Extra.LogAnalyticsWorkspaceIds -as [string]),'')
            }
    ...
    return [PSCustomObject]@{
        ...
        AccessPolicyDetails = NullCoalesce(($Analysis.Extra.AccessPoliciesDetails -as [string]),'')
...
function Collect-ExtraAzData {
    ...
    Write-Log "[DEBUG] Collect-ExtraAzData start for $resIdDbg" -Level 'INFO'
                ...
                try {
                    ...
                    try {
                        ...
                        foreach ($ra in $roleAssignments) {
                            ...
                            $roleName = NullCoalesce(($ra.RoleDefinitionName -as [string]),'')
                    ...
                    try {
                        ...
                        if ($kv -and $kv.AccessPolicies) {
                            foreach ($ap in $kv.AccessPolicies) {
                                try {
                                    $pname = NullCoalesce(($ap.DisplayName -as [string]),($ap.ObjectId -as [string]))
                                    $pobj = NullCoalesce(($ap.ObjectId -as [string]),'')
                                    $keys = @(); $secrets = @(); $certs = @()
                    ...
                    } catch { $Analysis.Extra.AccessPoliciesDetails = NullCoalesce($Analysis.Extra.AccessPoliciesDetails,'') }
                    ...
                    } catch { $Analysis.Extra.LogAnalyticsWorkspaceIds = NullCoalesce($Analysis.Extra.LogAnalyticsWorkspaceIds,'') }
                    } catch {
                    ...
                    if ($msg -and ($msg -match 'Public network access is disabled' -or $msg -match 'ForbiddenByConnection')) {
                        ...
                        $Analysis.Extra.SkuName = NullCoalesce($Analysis.Extra.SkuName,'')
                    }
        ...
        try {
            ...
            $Analysis.Extra.ManagedIdentityType = NullCoalesce($Analysis.Extra.ManagedIdentityType,'')
            $Analysis.Extra.ManagedIdentityIds = NullCoalesce($Analysis.Extra.ManagedIdentityIds,'')
            $Analysis.Extra.ManagedIdentityResolved = NullCoalesce($Analysis.Extra.ManagedIdentityResolved,'')
        }
        ...
        try {
            ...
            $Analysis.Extra.SecretRotationMostRecent = NullCoalesce($Analysis.Extra.SecretRotationMostRecent,'')
            $Analysis.Extra.KeyRotationMostRecent = NullCoalesce($Analysis.Extra.KeyRotationMostRecent,'')
        }
        ...
        if ($secrets) {
            ...
            foreach ($secret in $secrets) {
                $secretDetails += @{
                    ...
                    VaultName = $VaultName
                    BusinessUnit = NullCoalesce(NullCoalesce($vaultTags.Get_Item("BusinessUnit"),$vaultTags.Get_Item("BU")),"Unknown")
                    IAPMNumber = NullCoalesce(NullCoalesce($vaultTags.Get_Item("IAPM#"),$vaultTags.Get_Item("IAPM")),NullCoalesce($vaultTags.Get_Item("ProjectID"),"Unknown"))
                    Environment = NullCoalesce($vaultTags.Get_Item("Environment"),"Unknown")
...
"@
    ...
    foreach ($platform in $PlatformAssessments) {
        ...
        $safeJson = (Normalize-ForJson $platform | ConvertTo-JsonCompact -InputObject -Depth 4) -replace '</', '<\/'
    ...
    foreach ($vault in $AnalysisResults) {
        ...
        $vaultPayload = @{
            ...
            VaultScore = NullCoalesce(($vault.ComplianceScore -as [int]),0)
        ...
        try {
            $safeVaultJson = (Normalize-ForJson $vaultPayload | ConvertTo-JsonCompact -InputObject -Depth 4) -replace '</', '<\\/'
        } catch {
            ...
            $safeVaultJson = (Normalize-ForJson $fallback | ConvertTo-JsonCompact -InputObject -Depth 2) -replace '</', '<\\/'
        }
    ...
    $safePermsJson = (Normalize-ForJson $permsForEmbed | ConvertTo-JsonCompact -InputObject -Depth 4) -replace '</', '<\/'
...
function Invoke-GapAnalysis {
    ...
    if ($global:gapAnalysisResults -and $global:gapAnalysisResults.Count -gt 0) {
        ...
        try {
            ...
            try {
                $flattenedResults | ConvertTo-JsonCompact -InputObject -Depth 10 | Out-File -FilePath $finalCoercionTemp -Encoding UTF8 -Force
            ...
            $finalCoercionScript = {
                ...
                foreach ($orig in $rowsToProcess) {
                    ...
                    if ($propCount -eq 0 -and ($orig -is [string] -or $orig -is [System.ValueType] -or $rowType -eq 'System.Boolean')) {
                        try {
                            $co = [ordered]@{}
                            $co['Value'] = NullCoalesce(($orig -as [string]),'')
                    ...
                    try {
                        ...
                        $props['Value'] = NullCoalesce(($orig -as [string]),'')
                        ...
                        if ($null -eq $rawRole) {
                            ...
                            foreach ($elem in $rawRole) {
                                ...
                                if ($elem -is [PSObject] -or $elem -is [hashtable]) {
                                    try { $parts += (ConvertTo-JsonCompact -InputObject $elem -Depth 2) } catch { $parts += $elem.ToString() }
                                } else { $parts += $elem.ToString() }
                            }
                            $props['RoleAssignmentsResolved'] = ($parts | Where-Object { $_ -and $_ -ne '' }) -join '; '
                        } else {
                            $props['RoleAssignmentsResolved'] = NullCoalesce(($rawRole -as [string]),'')
                        }
                    } catch {
                        $props['RoleAssignmentsResolved'] = ''
                    }
                    ...
                    $props['JsonFilePath'] = NullCoalesce((Get-PropValueLocal $props 'JsonFilePath' -as [string]),'')
                    ...
                    try {
                        ...
                        if (-not $disablePerVaultJson -and $jsonPath -and $jsonPath -ne '') {
                            $vaultExport = [ordered]@{
                                Timestamp = (Get-Date).ToString('o')
                                SubscriptionId = NullCoalesce((Get-PropValueLocal $props 'SubscriptionId' -as [string]),'')
                                SubscriptionName = NullCoalesce((Get-PropValueLocal $props 'SubscriptionName' -as [string]),'')
                                VaultName = NullCoalesce((Get-PropValueLocal $props 'VaultName' -as [string]),'')
                                VaultResourceId = NullCoalesce((Get-PropValueLocal $props 'VaultResourceId' -as [string]),'')
                                Location = NullCoalesce((Get-PropValueLocal $props 'Location' -as [string]),'')
                                ComplianceScore = NullCoalesce((Get-PropValueLocal $props 'ComplianceScore' -as [int]),0)
                                VaultScore = NullCoalesce(($props['VaultScore'] -as [int]),0)
                                RoleAssignmentsResolved = NullCoalesce(($props['RoleAssignmentsResolved'] -as [string]),'')
                                ManagedIdentityResolved = NullCoalesce((Get-PropValueLocal $props 'ManagedIdentityResolved' -as [string]),'')
                                DiagnosticDestinationNames = NullCoalesce((Get-PropValueLocal $props 'DiagnosticDestinationNames' -as [string]),'')
                                SkuName = NullCoalesce((Get-PropValueLocal $props 'SkuName' -as [string]),'')
                                SecretRotationMostRecent = NullCoalesce((Get-PropValueLocal $props 'SecretRotationMostRecent' -as [string]),'')
                                KeyRotationMostRecent = NullCoalesce((Get-PropValueLocal $props 'KeyRotationMostRecent' -as [string]),'')
                    ...
                    try {
                        $vn = NullCoalesce((Get-PropValueLocal $props 'VaultName' -as [string]),'')
                    ...
                    try {
                        if ($props -is [hashtable] -or $props -is [System.Collections.Specialized.OrderedDictionary]) {
                            ...
                            try { $localFinal += [PSCustomObject]@{ Value = NullCoalesce(($orig -as [string]),'') } } catch { }
                ...
                try {
                    ...
                    foreach ($orig in $flattenedResults) {
                        try {
                            ...
                            $propsFb['Value'] = NullCoalesce(($orig -as [string]),'')
                        }
                        ...
                        try {
                            ...
                            } else { $propsFb['RoleAssignmentsResolved'] = NullCoalesce(($rawRole -as [string]),'') }
                        ...
                        # JsonFilePath
                        $propsFb['JsonFilePath'] = NullCoalesce(($propsFb['JsonFilePath'] -as [string]),'')
                        ...
                        try {
                            ...
                            if ($jsonPathFb -and $jsonPathFb -ne '') {
                                $vaultExportFb = [ordered]@{
                                    Timestamp = (Get-Date).ToString('o')
                                    SubscriptionId = NullCoalesce(($propsFb['SubscriptionId'] -as [string]),'')
                                    SubscriptionName = NullCoalesce(($propsFb['SubscriptionName'] -as [string]),'')
                                    VaultName = NullCoalesce(($propsFb['VaultName'] -as [string]),'')
                                    VaultResourceId = NullCoalesce(($propsFb['VaultResourceId'] -as [string]),'')
                                    Location = NullCoalesce(($propsFb['Location'] -as [string]),'')
                                    ComplianceScore = NullCoalesce(($propsFb['ComplianceScore'] -as [int]),0)
                                    VaultScore = NullCoalesce(($propsFb['VaultScore'] -as [int]),0)
                                    RoleAssignmentsResolved = NullCoalesce(($propsFb['RoleAssignmentsResolved'] -as [string]),'')
                        ...
                        $diagMsgsFb += NullCoalesce(("Processed vault: {0}" -f (($propsFb['VaultName'] -as [string]),'')))
